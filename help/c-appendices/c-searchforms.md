---
description: Search&amp;Promote의 검색 양식에 대해 알아봅니다.
solution: Target
title: 검색 양식
...
컬렉션을 사용하면 고객이 웹 사이트의 특정 영역을 검색할 수 있습니다. 드롭다운 목록을 구현하는지 또는 확인란 목록을 구현하는지에 따라 고객이 단일 컬렉션이나 여러 컬렉션을 검색하도록 할 수 있습니다.

[컬렉션 정보](../c-about-settings-menu/c-about-searching-menu.md#concept_62E42ACE53D54EEE9273433B86259127)도 참조하십시오.

다음 예제에서는 네 개의 서로 다른 컬렉션 이름과 웹 사이트에서 다루는 관련 영역을 보여 줍니다.
...
드롭다운 검색 양식 인터페이스를 사용하면 한 개의 컬렉션을 선택할 수 있으며 다음과 같이 표시됩니다.

![](assets/DropdownsearchformUI.png)

드롭다운 검색 양식은 다음 HTML 코드와 함께 생성됩니다.
...
또는 검색 양식에서 확인란 그룹을 사용하여 방문자가 여러 컬렉션을 선택할 수 있습니다.

![](assets/checkboxes.png)

확인란 검색 양식은 다음 HTML 코드로 생성됩니다.
...
`<search-input>` 태그는 `<input>` 태그를 출력하며, 컬렉션이 검색에 지정된 경우 `checked` 속성을 포함합니다.

## 양식에서 프레임 사용 {#reference_82CDDDA1E37042E4849EBF7EA05407C5}

사이트 검색/머천다이징에서 작동하도록 프레임세트를 구성할 수 있습니다.

<!-- 404 DEAD LINK To learn more about HTML frames and the HTML frameset element, see the following URL:

[https://www.w3schools.com/html/html_frames.asp](https://www.w3schools.com/html/html_frames.asp) -->

사이트에서 프레임을 사용하는 경우 검색 결과 링크에 대한 대상 프레임을 지정할 수 있습니다. 기본 타겟은 _self이며, 현재 프레임 또는 브라우저 창에서 링크를 엽니다. 대신 사이트별 또는 브라우저 예약 대상을 지정할 수 있습니다.

* _top (브라우저 예약) 결과를 현재 브라우저 창에서 열고 현재 프레임을 모두 바꿉니다.
* 빈(브라우저 예약) 결과 새 브라우저 창에서 열립니다(_B).
* _parent(browser-reserved) 를 선택하면 현재 프레임의 상위 프레임에 열립니다.
* frame2(사이트별)는 &quot;frame2&quot;라는 프레임에 열립니다. 프레임의 이름을 값(예: 주 또는 컨텐츠)으로 지정할 수 있습니다.

사이트에서 프레임을 사용하지 않는 경우 기본 대상 이름을 변경하지 않을 수 있습니다.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col02"> <p>검색 결과 페이지에 대한 대상 프레임을 설정합니다. </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col02"> <p>검색 결과 페이지에서 수행되는 링크의 대상을 설정합니다. </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col02"> <p>탐색 프레임 페이지를 편집하여 색인화되지 않도록 합니다. </p> </td> 
   <td colname="col2"> <p> <a scope="local" href="#section_C62E5F0EE1294D5EBD97E123E54433FC" type="section" format="dita"> 탐색 프레임 페이지를 편집하여 이러한 페이지가 표시되지 않도록 하는 중...  </a> </p> </td> 
  </tr> 
  <tr> 
...
1. 검색 양식 코드를 프레임세트에서 원하는 프레임에 붙여넣습니다.

   아래 예제에서 검색 양식 코드는 탐색 프레임인 화면 왼쪽에 있는 좁은 세로 프레임으로 붙여넣습니다.

   ![](assets/frames1.gif)

## 검색 결과 페이지에 대한 대상 프레임 설정 {#section_532CACB90888467093D95EACB64FDFA1}

검색 양식 코드를 위와 같이 세로 탐색 프레임에 배치한 경우 검색 결과를 더 큰 기본 프레임에 표시할 수 있습니다. 이 예에서는 기본 프레임 &quot;body&quot;를 호출하고 대상 프레임으로 설정합니다.

![](assets/frames2.gif)

1. 결과 페이지의 대상 프레임을 지정하려면 검색 양식 코드의 다음 행을 다음과 같이 변경하여 양식에 대상 및 값을 추가합니다.
...
고객이 웹 사이트를 검색하면 검색 결과가 웹 페이지의 &quot;본문&quot; 프레임에 나타납니다.

## 검색 결과 페이지에서 만든 링크에 대한 대상 설정 {#section_523248C5AC424D878321C21A23A5CD66}

템플릿을 직접 편집하여 대상 프레임을 설정할 수 있습니다.

검색 결과가 &quot;body&quot; 프레임에 나타나는 경우 링크가 &quot;body&quot; 프레임에서도 열리도록 할 수 있습니다. 프레임은 동일하므로 기본 설정인 대상 값 `"_self"` 을 변경할 필요가 없습니다.

결과 링크에 대한 대상 프레임을 설정할 수도 있습니다. 다음은 수행할 수 있는 작업의 몇 가지 예입니다.

* 클릭한 각 결과가 별도의 프레임에서 열리는 동안 검색 결과가 자체 프레임에서 활성 상태로 유지되도록 검색 결과 및 해당 링크에 대해 다른 프레임을 지정합니다.
* 검색 결과가 새 빈 창에 열리도록 지정하여 이전 창이 원래 콘텐츠로 활성화되고 검색 결과도 보존되도록 합니다.

대상 이름은 HTML에 지정된 프레임의 이름이거나 다음 HTML 기본값 중 하나일 수 있습니다.

* `target="_blank"` 이름이 지정되지 않은 새 창에서 링크를 엽니다.

* `target="_self"` 기본값. 검색 결과가 있는 동일한 창에서 링크를 엽니다. 이 경우 원래 검색 결과 창입니다. 전역 지정된 기본 대상을 재정의하려면 이 옵션을 사용합니다.

* `target="_parent"` 링크 페이지의 상위 프레임세트에서 링크를 엽니다. 문서에 상위 항목이 없으면 기본적으로 `"_self"` 과 같은 기능이 작동합니다.

* `target="_top"` 전체 창에서 링크를 엽니다. 문서가 이미 맨 위에 있는 경우 기본적으로 `"_self"` 과 같은 기능이 수행됩니다. 임의의 딥 프레임 중첩에서 분리하려면 이 옵션을 사용합니다.

예를 들어 `_blank` 대상 프레임을 설정하려면 다음 방법으로 템플릿을 편집할 수 있습니다.

1. 제품 메뉴에서 **[!UICONTROL Design]** > **[!UICONTROL Templates]** 를 클릭합니다.

1. [!DNL Staged Templates] 페이지의 테이블에서 대상 프레임이 있는 템플릿의 이름을 클릭합니다.
1. `<search-link>` 태그를 찾습니다. 기본 `<search-link>` 태그는 다음과 유사해야 합니다.

   `<search-link><search-title length=100></search-link>`

1. 프레임 대상을 `<search-link>` 태그에 추가합니다. 위의 예에서 `target="_blank"`을 입력합니다. 대상 값 주위에 밑줄과 따옴표를 포함해야 합니다.

   이제 `<search-link>` 태그가 다음과 같이 표시됩니다.

   `<search-link target="_blank"><search-title length=100></search-link>`

사이트 방문자가 검색 결과 링크를 선택하면 이제 연결된 페이지가 새 빈 창으로 열립니다.

## 탐색 프레임 페이지를 편집하여 색인화되지 않도록 합니다 {#section_C62E5F0EE1294D5EBD97E123E54433FC}

일반적으로 탐색 프레임이 검색 결과로 인덱싱되지 않도록 제외할 수 있습니다. 이 기능을 수행하기 위해 해당 페이지에 `noindex` 메타 태그를 추가할 수 있습니다.

1. 탐색 프레임의 HTML 페이지 소스를 엽니다.
...
## 샘플 고급 검색 양식 {#reference_82E1051918744EBA88A01E9E6AE42C4A}

디자인 및 콘텐츠 요구 사항에 맞게 고급 양식 코드를 편집하거나 추가 검색 매개 변수를 추가하거나 제거할 수 있습니다.

많은 고객이 본인의 홈페이지에서 검색 기능을 찾길 원하므로 홈페이지는 고급 검색 양식을 삽입하기에 좋은 장소입니다. 검색 양식 및 기타 유용한 정보를 포함하는 HTML 페이지를 만든 다음 웹 사이트 전체에서 해당 페이지에 연결할 수도 있습니다.

보안 컨텐츠를 인덱싱하는 경우 보안 검색 웹 서버에서 검색 결과를 제공할 수 있습니다. 검색 양식 작업 특성의 URL을 다음과 같이 변경합니다. action=&quot;https://search.atomz.com/search/&quot;

>[!NOTE]
>
>일부 HTML 편집기에서 다른 애플리케이션에서 HTML 코드를 붙여넣는 데 문제가 있습니다. HTML 코드가 웹 페이지에 텍스트로 표시되면 검색 코드를 복사하여 Windows의 메모장 또는 Mac의 단순 텍스트 등의 간단한 텍스트 편집기에 붙여넣은 다음 단순 텍스트 편집기에서 HTML 편집기로 다시 복사하여 붙여넣습니다.

검색 매개 변수는 고급 검색 양식 코드에서 고객이 개별 검색을 사용자 지정하는 데 사용할 수 있는 라디오 단추, 확인란 및 목록 상자를 만드는 데 사용됩니다. 고객은 표시되는 검색 결과 수(예: 또는 날짜 범위)를 지정하거나, 고급 검색 양식에 나타나는 검색 결과-모두 통과 옵션과 함께 요약을 표시할지 여부를 지정할 수 있습니다.
...
<table> 
 ...
 <tbody> 
  <tr> 
   <td colname="col2"> <p>고급 검색 양식 옵션(숨김 필드)을 활성화합니다 </p> </td> 
   <td colname="col1"> <p> <span class="codeph"> sp_advanced  </span> </p> </td> 
   <td colname="col3"> <p> <span class="syntax html codeph"> &lt;input type="hidden" name="sp_advanced" value="1"&gt; </span> </p> </td> 
   <td colname="col4"> <p>고급 검색 옵션을 활성화하거나 비활성화합니다. 예를 들어, 고급 양식이 포함된 두 번째 페이지에 대한 링크와 함께 홈 페이지에 표준 검색 양식을 넣을 수 있습니다. 이 경우 표준 양식의 사본을 <span class="codeph"> &lt;search-if-not-advanced&gt; 내에 넣습니다...&lt;/search-if-not-advanced&gt; </span> 템플릿 태그. </p> <p>표준 양식에서 검색을 수행하는 고객은 검색 결과가 표시될 때 표준 검색 양식이 표시됩니다. 고급 검색 양식 화면에서 <span class="codeph"> &lt;input type=hidden name="sp_advanced" value=1&gt; </span> 태그를 다른 고급 양식 옵션과 함께 포함합니다. </p> <p>또한 &lt;search-if-advanced&gt;.. &lt;/search-if-advanced&gt; 템플릿 태그 내에 고급 검색 양식의 사본을 포함합니다. 고급 검색 양식에서 검색을 수행하는 고객은 검색 결과가 표시될 때 고급 검색 양식을 봅니다. </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col4"> <p>일치하는 문서에 대해 "임의의 단어", "모든 단어" 또는 "정확한 구"를 표시하도록 고객이 지정할 수 있도록 합니다. <span class="codeph"> sp_p </span> 매개 변수가 지정된 경우 고객은 검색 쿼리에서 "+" 또는 "-" 또는 둘 다 사용할 필요가 없습니다. </p> <p> <span class="codeph"> sp_p </span> 매개 변수를 생략하거나 "" 또는 "any"로 설정한 경우 고객은 여전히 "+" 및 "-" 지정자를 사용할 수 있습니다. <span class="codeph"> sp_p </span> 매개 변수가 "all" 또는 "phrase"로 설정된 경우 지정된 "+" 및 "-"가 무시됩니다. </p> <p>검색에서 "+" 및 "-"를 사용하는 방법에 대해 자세히 알아볼 수 있습니다. </p> <p><a href="../c-about-settings-menu/c-about-searching-menu.md#concept_207105CF26B1448F8A3D223787C56AB8" type="concept" format="dita" scope="local">검색 정보</a>를 참조하십시오 . </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col4"> <p>고객이 유사 일치를 활성화하거나 비활성화할 수 있습니다. 유사 일치 기능을 사용하면 철자가 잘못된 검색 쿼리를 사용하여 문서에서 "유사"인 단어를 일치시킬 수 있습니다. </p> <p><span class="codeph"> sp_w_control </span> 매개 변수가 1로 설정되고 <span class="codeph"> sp_w </span> 매개 변수가 "동일함"으로 설정된 경우 생성된 확인란이 선택되고 기본적으로 유사 항목 일치가 활성화됩니다. </p> <p><span class="codeph"> sp_w </span> 매개 변수가 ""로 설정된 경우 확인란이 선택되지 않습니다. </p> <p>가장 최근 인덱싱 작업 중에 유사 일치를 활성화하지 않은 경우 유사 일치를 찾을 수 없으며 <span class="codeph"> sp_w </span> 매개 변수는 무시됩니다. 유사 일치를 활성화하려면 제품 메뉴에서 <span class="uicontrol"> 언어학 </span> &gt; <span class="uicontrol"> 단어 및 언어 </span> &gt; <span class="uicontrol"> 음성-유사 일치 </span>를 클릭하십시오. </p> <p>매개 변수 <span class="codeph"> sp_w </span> 및 <span class="codeph"> sp_w_control </span> 매개 변수를 다음과 같은 방법으로 할당할 수도 있습니다. </p> <p> <code class="syntax html"> &lt;!--&nbsp;Checkbox&nbsp;disables&nbsp;sound-alike&nbsp;matching&nbsp;--&gt; 
      &lt;input&nbsp;type=hidden&nbsp;name="sp_w_control"&nbsp;value=0&gt; 
      &lt;input&nbsp;type=checkbox&nbsp;name="sp_w"&nbsp;value="exact"&gt; 
      No&nbsp;sound-alike&nbsp;matching </code> </p> <p>이 경우 <span class="codeph"> sp_w_control </span> 매개 변수가 0으로 설정되고 <span class="codeph"> sp_w </span> 매개 변수가 "exact"로 설정되면 기본적으로 동일한 사운드가 비활성화됩니다. <span class="codeph"> sp_w </span> 매개 변수가 ""로 설정된 경우 유사 음성 일치 기능이 활성화됩니다. </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col4"> <p><span class="codeph"> sp_d </span> 매개 변수는 수행할 사용자 지정 데이터 범위 일치 또는 수행할 특정 날짜 범위를 지정합니다. </p> <p>기본 고급 검색 양식에서 이 옵션은 <span class="codeph"> sp_date_range </span> 매개 변수와 함께 생성된 "사용자 지정" 날짜 범위의 드롭다운 목록이 있는 라디오 단추 그룹으로 표시됩니다. 또한 <span class="codeph"> sp_start_day </span>, <span class="codeph"> sp_start_month </span>, <span class="codeph"> sp_start_year </span>, <span class="codeph"> sp_end_day </span>, <span class="codeph"> sp_end_month </span> 및 <span class="codeph"> sp_end_year </span> 매개 변수와 함께 생성되는 "specific" 시작 및 종료 날짜 그룹이 포함되어 있습니다. </p> <p>사용자 지정 날짜 범위는 검색할 이름이 지정된 날짜 범위입니다. 예를 들어 "Anytime", "Today", "Within the last year" 등이 있습니다. </p> <p>특정 날짜 범위는 시작 날짜와 종료 날짜로 구성됩니다. 예를 들어 "2009년 9월 8일부터 2011년 10월 18일까지"입니다. </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col3"> <p> <code class="syntax html"> &lt;!--Selection&nbsp;list&nbsp;for&nbsp;custom&nbsp;date&nbsp;range.--&gt; 
      ...
      &lt;/select&gt; </code> </p> </td> 
   <td colname="col4"> <p><span class="codeph"> sp_date_range </span> 매개 변수를 사용하여 "사용자 지정" 날짜 범위를 만듭니다. 예를 들어 "Anytime", "Today", "Within the last year" 등이 있습니다. </p> <p>0보다 크거나 같은 값은 오늘 전에 검색할 일 수를 지정합니다. 예를 들어 값이 0이면 "Today"를 지정하고, 값이 "1"이면 "Today and Yesterday"를 지정하고, 값이 "30"이면 "Within the Last 30일" 등을 지정합니다. 0보다 작은 값은 다음과 같이 사용자 지정 범위를 지정합니다. </p> <p> 
     <ul id="ul_E65DDE33883F441F9730F315E485AD98"> 
      ...
      <li id="li_38AB8D97179A47F9B860A96EA09119BB"> <p>-2 = "이번 주"로서, 현재 주의 일요일부터 토요일까지 검색합니다. </p> </li> 
      <li id="li_F4C3A8658428418A8A06FBAAB4733C68"> <p>-3 = "지난 주"로서, 이번 주 이전 주의 일요일부터 토요일까지 검색합니다. </p> </li> 
      <li id="li_DF2D0B043A4E4DE9BE8D82E69A76E793"> <p>-4 = "이번 달"이며, 현재 월 내의 날짜를 검색합니다. </p> </li> 
      <li id="li_76BC4C2CED574E2A81448158828BFF1B"> <p>-5 = "지난달"이며, 현재 월 이전 달 내의 날짜를 검색합니다. </p> </li> 
      <li id="li_17FF849384FB46D58AF6FF1D3BC408C8"> <p>-6 = "This year"로, 현재 연도 내의 날짜를 검색합니다. </p> </li> 
      <li id="li_E2B8B4DFF3914BBDB86D0EB77F52B305"> <p>-7 = "Last year"로, 현재 연도 이전 연도 내의 날짜를 검색합니다. </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col3"> </td> 
   <td colname="col4"> <p>이 숫자 값 세 항목은 검색할 특정 날짜 범위의 시작 날짜를 지정합니다. 부분적으로 지정된 날짜가 무시되므로 세 값을 모두 지정해야 합니다. </p> <p>시작 날짜만, 종료 날짜만 또는 시작 날짜와 종료 날짜를 모두 지정하는 것이 좋습니다. 시작 날짜만 지정한 경우 검색 시 시작 날짜 또는 이후 날짜가 지정된 일치하는 문서가 포함됩니다. 종료 날짜만 지정한 경우 검색 결과에 종료 날짜 또는 이전 버전의 일치하는 문서가 포함됩니다. 시작 날짜와 종료 날짜가 모두 지정된 경우 검색 결과에 시작 날짜부터 종료 날짜까지의 일치하는 문서가 포함됩니다. </p> <p>모든 날짜는 그리니치 표준시 기준으로 검색됩니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col2"> <p> 날짜 범위 일치: 종료 날짜 </p> </td> 
   <td colname="col1"> <p> <span class="codeph"> sp_end_day, sp_end_month, sp_end_year  </span> </p> <p> </p> </td> 
   <td colname="col3"> </td> 
   <td colname="col4"> <p>이 숫자 값 세 항목은 검색할 특정 날짜 범위의 종료 날짜를 지정합니다. 부분적으로 지정된 날짜가 무시되므로 세 값을 모두 지정해야 합니다. </p> <p>시작 날짜만, 종료 날짜만 또는 시작 날짜와 종료 날짜를 모두 지정하는 것이 좋습니다. 시작 날짜만 지정한 경우 검색 시 시작 날짜 또는 이후 날짜가 지정된 일치하는 문서가 포함됩니다. 종료 날짜만 지정한 경우 검색 결과에 종료 날짜 또는 이전 버전의 일치하는 문서가 포함됩니다. 시작 날짜와 종료 날짜가 모두 지정된 경우 검색 결과에 시작 날짜부터 종료 날짜까지의 일치하는 문서가 포함됩니다. </p> <p>모든 날짜는 그리니치 표준시 기준으로 검색됩니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col2"> <p>검색 필드 내에서 </p> </td> 
   ...
   <td colname="col3"> <p> <code class="syntax html"> &lt;!--&nbsp;List&nbsp;box&nbsp;selects&nbsp;the&nbsp;search&nbsp;field&nbsp;--&gt; 
      ...
      &lt;/select&gt; </code> </p> </td> 
   <td colname="col4"> <p><span class="codeph"> sp_x </span> 목록 상자를 사용하면 고객이 쿼리 문자열을 검색할 필드를 지정할 수 있습니다. </p> <p>고객은 모든 필드, 제목, 문서 설명, 문서 키워드, 본문, 대체 텍스트, 문서의 URL, 날짜 또는 대상 키워드를 선택할 수 있습니다. </p> <p><span class="codeph"> sp_x </span> 매개 변수를 사용하는 경우 고객은 검색 쿼리 문자열에서 "title:," "desc:," "keys:" "body:" "alt:", "url:" 및 "target:"을 지정할 필요가 없습니다. </p> <p><span class="codeph"> sp_x </span> 매개 변수를 생략하거나 "" 또는 "any"로 설정한 경우 고객은 여전히 필드 지정자 문자열을 사용할 수 있습니다. <span class="codeph"> sp_x </span> 매개 변수가 특정 필드로 설정된 경우 다른 모든 필드 지정자 문자열이 무시됩니다. </p> <p><a href="../c-about-settings-menu/c-about-searching-menu.md#concept_207105CF26B1448F8A3D223787C56AB8" type="concept" format="dita" scope="local">검색 정보</a>를 참조하십시오 . </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col4"> <p>고객이 각 일치 항목에 대해 요약 텍스트를 표시할지 여부를 선택할 수 있습니다. </p> <p>요약을 표시하려면 값을 1로 설정합니다. 요약을 숨기려면 값을 0으로 설정합니다. 다음 예와 같이 라디오 단추 세트와 함께 매개 변수를 사용할 수도 있습니다. </p> <p> <code class="syntax html"> &lt;!--&nbsp;Show&nbsp;or&nbsp;hide&nbsp;summaries&nbsp;in&nbsp;search&nbsp;results&nbsp;--&gt; 
  ...
  <tr> 
   ...
   <td colname="col3"> <p> <code class="syntax html"> &lt;!--&nbsp;Sort&nbsp;results&nbsp;by&nbsp;relevance&nbsp;or&nbsp;by&nbsp;date&nbsp;--&gt; 
      ...
      &lt;/select&gt; </code> </p> </td> 
   <td colname="col4"> <p>고객이 결과를 관련성 또는 날짜 순서로 나열할지 선택할 수 있습니다. </p> <p>이 값을 1로 설정하면 가장 최근에 변경된 문서에서 가장 최근에 변경된 문서로 결과가 나열됩니다. 이 값을 0으로 설정하면 가장 관련성이 낮은 결과부터 결과가 나열됩니다. 다음 예와 같이 라디오 단추와 함께 이 매개 변수를 사용할 수도 있습니다. </p> <p> <code class="syntax html"> &lt;!--&nbsp;Sort&nbsp;results&nbsp;by&nbsp;relevance&nbsp;or&nbsp;by&nbsp;date&nbsp;--&gt; 
...
[샘플 고급 검색 양식](#reference_82E1051918744EBA88A01E9E6AE42C4A)을 참조하십시오.

이 코드를 사용하는 경우 `sp99999999`의 `sp_a` 값을 실제 계정 번호로 바꾸십시오.

계좌 번호를 찾으려면 제품 메뉴에서 **[!UICONTROL Settings]** > **[!UICONTROL Account Options]** > **[!UICONTROL Account Settings]**&#x200B;를 클릭하십시오.
...
모든 매개 변수에 대한 기본 선택 사항이 이전 검색과 동일한 방식으로 고급 검색 양식 HTML 코드를 템플릿에 추가할 수 있습니다.
